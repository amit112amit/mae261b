%Verification tests for T3Lin element
...
for index=1:1000
    ...
    fprintf('Running verification tests for T3Lin element...\n');
    ...
    if (abs(sum(DN(:,1)))>tol1 || abs(sum(DN(:,1)))> tol1)
    ...
    p_a = [p(1,1);p(0,0);p(1,0)];
    [N_a,~] = T3Lin(theta_star);
...
fprintf('Verification of T3Lin element completed.\n');